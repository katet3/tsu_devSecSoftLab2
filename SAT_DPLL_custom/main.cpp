/*
    Используемые компоненты
        BoolEquation — представляет КНФ (конъюнктивную нормальную форму) и хранит:
        Массив дизъюнктов (BoolInterval** cnf_ptr_).
        Корневой интервал (BoolInterval* root_ptr_).
        Маску переменных (BBV mask_).
        Стратегию ветвления (BranchingStrategy).

    NodeBoolTree — узел бинарного дерева решений (хранит BoolEquation и ссылки на поддеревья lt, rt).
    BoolInterval — представляет интервал значений переменных (аналог "частичного присваивания").
    BBV (Bit-Block Vector) — битовая маска для управления переменными.
*/

/*
    Алгоритм
    Алгоритм работает по схеме «разделяй и властвуй» с правилами упрощения и отсечением нерешаемых веток
    
    1. Проверка тривиальных случаев (CheckRules)
    Перед ветвлением вызывается equation_ptr_->CheckRules(), который проверяет:
        0 (нет решения) — если какой-то дизъюнкт стал невыполнимым.
        1 (найдено решение) — если все дизъюнкты выполнены (count == 0 или mask полностью заполнена).
        2 (нужно ветвление) — если упрощение невозможно.

    2. Упрощение (Simplify)
    Если правило 2, выбирается переменная для ветвления (ChooseBranchingIndex), и уравнение упрощается:
        Ветка 0: equation.Simplify(branch_index_, '0').
        Ветка 1: equation.Simplify(branch_index_, '1').
    Упрощение включает:
        Удаление выполненых дизъюнктов.
        Сокращение оставшихся (например, если x=1, то дизъюнкт (x ∨ y) становится true).

    3. Рекурсивное ветвление
    Создаются два поддерева

    4. Обход в глубину (DFS)
    Используется стек для обхода

    5. Восстановление решения
    Если найдено решение (count == 0), корневой BoolInterval содержит значения переменных
*/
